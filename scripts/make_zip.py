...
def  main():
  ...
  installer = "\installer\Flicker Installer.exe"
   
  if demo:
    installer = "\installer\Flicker Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Flicker manual.pdf" 
  ]
  ...
  ZIPNAME = "Flicker-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Flicker-v" + FULLVERSIONSTR + "-win-demo.zip"
